# Optimized version of main.py to generate st flow trajectories
"""
...
y=x
z=x
positions=np.mgrid[-15.0:15.0:31j,
                      0:0:31j,
                      0:15:31j]
xx, yy, zz = positions
...
singularity_pos = lambda t: np.array([np.cos(t),0.0,np.sin(t)])
singularity_position=singularity_pos(TT)
singularity_velocity = lambda t: np.array([-np.sin(t),0.0,np.cos(t)])
...
for i in range(xx.size):
    ...
    vel = stokeslet_vec(X-singularity_pos(0), singularity_velocity(0))
    ux.flat[i] = vel[0]
    uy.flat[i] = vel[1]
    uz.flat[i] = vel[2]
pltxx = np.reshape(xx[:,0,:],(xx.shape[0],-1))
pltyy = np.reshape(yy[:,0,:],(xx.shape[0],-1))
