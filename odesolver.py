...
def singularity_velocity20(t):
    ...
    if t <= 0.25:
        ...
        return 4*np.pi*np.array([0.0,np.sin(4*np.pi*(0.25-t)),-np.cos(4*np.pi*(0.25-t))])
    else:
        return np.array([0,0,0])
...
def velocity2(t,y):
    return stokeslet_vector2(y-singularity_pos2(t),t)+stokeslet_vectorRrr(y-singularity_pos2(t-0.25),t-0.25)
...
r4=odes.ode(velocity2).set_integrator("lsoda")
r4.set_initial_value(np.array([5,0,0]),0.0)
for i in range(len(list(TT))):
...
ax.plot(trajectories[0][:,0],trajectories[0][:,1],trajectories[0][:,2],lw=0.5)
...
ax.plot(singularity_position2[:,0],singularity_position2[:,1],singularity_position2[:,2],lw=0.5)
